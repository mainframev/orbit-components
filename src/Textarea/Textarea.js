...
`;
const Area = styled(({ theme, tokens, size, error, help, ...props }) => <textarea {...props} />)`
  appearance: none;
...
const Textarea = (props: Props) => {
  const { size = SIZE_OPTIONS.NORMAL, theme = defaultTokens } = props;
  ...
  return (
    <Field theme={theme}>
      {props.label && (
        <FormLabel theme={theme} filled={!!props.children} disabled={props.disabled}>
          {props.label}
        </FormLabel>
      )}
      <Area
